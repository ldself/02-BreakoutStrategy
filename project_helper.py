...
def plot_lookahead_prices(prices, lookahead_price_list, title):
    ...
    for (lookahead_prices, lookahead_days), color in zip(lookahead_price_list, colors, strict=False):
        traces.append(
            go.Scatter(
...
def plot_price_returns(prices, lookahead_returns_list, title):
    ...
    traces = [_generate_stock_trace(prices)]
    for (lookahead_returns, lookahead_days), color in zip(lookahead_returns_list, colors, strict=False):
        traces.append(
            go.Scatter(
...
def plot_signal_returns(prices, signal_return_list, titles):
    ...
    stock_trace = _generate_stock_trace(prices)
    for (signal_return, signal, lookahead_days), color, title in zip(signal_return_list, colors, titles, strict=False):
...
def plot_signal_histograms(signal_list, title, subplot_titles):
    ...
    y_range = [0, 1500]
    config = helper.generate_config()
    colors = Color(helper.color_scheme['low_value']).range_to(Color(helper.color_scheme['high_value']),
                                                                    len(signal_series_list))
    ...
    for series_i, (signal_series, color) in enumerate(zip(signal_series_list, colors, strict=False), 1):
